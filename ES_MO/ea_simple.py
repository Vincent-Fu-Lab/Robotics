...
def ea_simple(n, nbgen, evaluate, IND_SIZE, weights=(-1.0,)):
    ...
    for g in range(1,nbgen):
        ...
        for i in range((n + 1) // 2):
            ind1, ind2 = random.randint(0, len(pop) - 1), random.randint(0, len(pop) - 1)
            
            while ind2 == ind1:
                ind2 = random.randint(0, len(pop) - 1)
            
            p1, p2 = toolbox.crossover(toolbox.clone(pop[ind1]), toolbox.clone(pop[ind2]))
        ...
        pop = toolbox.selection(pop + offspring)
